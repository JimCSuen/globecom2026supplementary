...
import numpy as np
from .satellite import Satellite, ServiceSatellite
...
class Orbit:
    ...
    def __init__(self, constellation, id, radius, inclination, num_sats):
        ...
        self.phase_offset = self.constellation.phasediff * (
            self.id - 1
        )  # Phase offset for the orbit compared to orbit 1
        if self.constellation.type == "Walker Delta Constellation":
            self.right_ascension = (
                2 * np.pi * (id - 1) / constellation.num_orbits
            )  # Longitude of the ascending node
        elif self.constellation.type == "Walker Star Constellation":
            self.right_ascension = (
                1 * np.pi * (id - 1) / constellation.num_orbits
            )  # Longitude of the ascending node
...
class ServiceOrbit(Orbit):
    """
    Class representing an orbit in a satellite constellation.
    """

    def __init__(self, constellation, id, radius, inclination, num_sats, fs):
        """
        Initialize the orbit's properties.

        Parameters:
        - constellation: WalkerConstellation object representing the satellite constellation.
        - fs: CPU frequency of the satellites in this orbit
        - id: Orbit ID.
        - radius: Radius of the orbit in km.
        - inclination: Inclination of the orbit in radians.
        - num_sats: Number of satellites in the orbit.
        """
        super().__init__(constellation, id, radius, inclination, num_sats)
        self.fs = fs
        self.sats = [ServiceSatellite(i + 1, self, fs) for i in range(num_sats)]
        return
