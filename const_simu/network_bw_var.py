...
class ServiceSatNetDelta(SatNet):
    ...
    def V_test(self):
        ...
        with open("check_point/exp_data/lyap_MPC_record_V_test.json", "w") as f:
            f.write(f"{self.lyap_MPC_record.record_list}")
        self.original_V = self.V
        for self.V in range(1, 500, 1):
            self.new_task_alloc = []
    ...
    def const_size_test(self):
        ...
        with open("check_point/exp_data/rand_record_const_size.json", "w") as f:
            f.write(f"{self.rand_record.record_list}")
        with open("check_point/exp_data/nearest_record_const_size.json", "w") as f:
            f.write(f"{self.nearest_record.record_list}")
        with open("check_point/exp_data/nearest_gnd_record_const_size.json", "w") as f:
            f.write(f"{self.nearest_gnd_record.record_list}")
        with open("check_point/exp_data/nearest_gnd_data_const_size.json", "w") as f:
            f.write(f"{self.nearest_gnd_record.record_list}")

        # ! loop for const size, given task size & number
        for N_const in range(5, 9, 1):
            self.new_task_alloc = []
            self.occupied_full_history = []
            ...
            with open("check_point/exp_data/nearest_record_const_size.json", "a") as f:
                f.write(f"{self.nearest_record.record_list}")
            with open(
                "check_point/exp_data/nearest_gnd_record_const_size.json", "a"
            ) as f:
                f.write(f"{self.nearest_gnd_record.record_list}")
            with open(
                "check_point/exp_data/nearest_gnd_data_const_size.json", "a"
            ) as f:
                f.write(f"{self.nearest_gnd_record.record_list}")
    ...
    def task_num_test(self):
        ...
        with open("check_point/exp_data/nearest_gnd_record_task_num.json", "a") as f:
            f.write(f"{self.nearest_gnd_record.record_list}")
        with open("check_point/exp_data/nearest_gnd_data_task_num.json", "a") as f:
            f.write(f"{self.nearest_gnd_record.record_list}")

        # ! loop for task number, given const size & task size
        for task_num in range(6, 30, 1):
            self.graph_reset()
            self.new_task_alloc = []
            self.occupied_full_history = []
            ...
            self.task_number_vector = np.int32(
                self.rng.uniform(
                    low=task_num / 2, high=task_num * 1.5, size=int(self.max_time_span)
                )
            )
            ...
            with open("check_point/exp_data/nearest_record_task_num.json", "a") as f:
                f.write(f"{self.nearest_record.record_list}")
            with open(
                "check_point/exp_data/nearest_gnd_record_task_num.json", "a"
            ) as f:
                f.write(f"{self.nearest_gnd_record.record_list}")
            with open("check_point/exp_data/nearest_gnd_data_task_num.json", "a") as f:
                f.write(f"{self.nearest_gnd_record.record_list}")
    ...
    def vectorized_MPC(self, time_step):
        ...
        MPC_cost = self.vectorized_MPC_cvx_problem.solve(verbose=True, solver=cp.GUROBI)
    ...
    def vectorized_history_update(self):
        ...
        link_load_ratio_mat = np.multiply(
            ...
            1 / self.vectorized_full_link_bw_mat,
        )
        lyap_MPC_link_LB = cp.sum_squares(link_load_ratio_mat).value / len(
            self.graph.edges()
        )
        ...
        print(f"this is the pwq update checking: {self.lyap_MPC_pwq_matrix}")
    ...
    def create_vectorized_MPC_cvx_constr(self, input_ts):
        ...
        self.vectorized_MPC_cvx_constr += [
            self.vectorized_bw_vector >= self.bw_min,
            self.vectorized_bw_vector <= self.bw_max,
        ]
        ...
        # ! thie mccormick is for the obj & C9
        # ! test comment
        self.vectorized_MPC_cvx_constr += (
            [self.vectorized_auxiliary >= self.bw_min * self.vectorized_routing_vector]
            + [
                self.vectorized_auxiliary
                >= self.bw_max * self.vectorized_routing_vector
                + cp.kron(self.vectorized_bw_vector, base_one_mat).T
                - self.bw_max * large_base_one_mat
            ]
            + [
                self.vectorized_auxiliary
                <= self.bw_max * self.vectorized_routing_vector
            ...
            + [
                self.vectorized_auxiliary
                <= self.bw_min * self.vectorized_routing_vector
                + cp.kron(self.vectorized_bw_vector, base_one_mat).T
                - self.bw_min * large_base_one_mat
            ]
        )
        ...
        for tp in range(
                ...
                if mem_time_step >= 0:
                    if mem_time_step < input_ts:  # ! has memory
                        ...
                        self.vectorized_trans_time_auxiliary.append(
                            cp.Variable(
                                (self.T_pred, self.N_max * self.constellation.num_sats),
                                boolean=True,
                            )
                        )
                        # * no future MPC
            ...
            if tp == 0:  # ! bw of the first slot is the last time
                ...
                #     * self.constellation.num_sats,
                # ] = self.vectorized_BW_history
                # self.vectorized_BW.append(self.vectorized_BW_history)
                self.vectorized_BW.append(copy.deepcopy(self.vectorized_BW_history))
                # print(f"LAST TIME:\n{self.vectorized_BW}")
            else:
                self.vectorized_BW.append(self.vectorized_BW[-1])
                # self.vectorized_BW.append(copy.deepcopy(self.vectorized_BW[-1]))
    ...
    def create_vectorized_MPC_cvx_vars(self):
        self.vectorized_bw_vector = cp.Variable(
            (self.T_pred * self.N_max * self.constellation.num_sats, 1)
        )
        ...
        self.vectorized_auxiliary = cp.Variable(  # * same size as routing vector
            (
                self.constellation.num_sats,
                self.T_pred
                * self.N_max
    ...
    def one_term_full_timescale_test(self, task_num, N_const, V_test):
        ...
        for i in range(len(self.task_number_vector)):
            # for i in range(8):
            print(
                f":::::::::::::::::::THIS IS ({"IS" if V_test else "NOT"} V-test): {task_num} tasks, {N_const}-Constellation, SLOT {i}/{self.max_time_span}::::::::::::::::::::"
            )
    ...
    def nearest_gnd_update(self):
        ...
        for task in self.selected_sat_service:
            ...
            if if_offload:
                ...
                temp_max_bw = 1e30
                for i in range(len(path) - 1):
                    temp_max_bw = min(temp_max_bw, residue_bw[(path[i], path[i + 1])])
                temp_bw = self.rng.uniform(
                    low=self.bw_min, high=min(temp_max_bw, self.bw_max)
                )
        ...
        for key, val in self.rand_task_alloc.items():
            ...
            for sat in self.graph.nodes():
                ...
                if (sat != src_sat) and (
                    ...
                    for i in range(len(path) - 1):
                        temp_max_bw = min(
                            temp_max_bw, rand_bw_residue[(path[i], path[i + 1])]
                        )

                    temp_bw = self.rng.uniform(
                        low=self.bw_min,
                        high=min(temp_max_bw, self.bw_max),
                        # low=self.bw_min, high=temp_max_bw #! or maybe use this
                    )
    ...
    def solve_MPC_comm(self, i):
        # * duplicate the last slot data padding the future prediction
        ...
        # * solve
        result = self.cvx_MPC_full_prob.solve(verbose=False, solver=cp.GUROBI)
        print(f"MPC RESULT IS HERE: {result}")
        return result

    def history_update(self):
        # history_leaving = {}
        # new_arriving = {}
        avg_link_load_balancing = 0
        lyap_MPC_MLLR = 0
        for s, d in self.graph.edges():
            ...
            #! update PHY workload queue
            for orbit_id in range(len(self.constellation.orbits)):
                orbit = self.constellation.orbits[orbit_id]
                for sat_id in range(len(orbit.sats)):
                    sat = orbit.sats[sat_id]
                    sat.pwq = max(0, sat.pwq - sat.fs)
                    for t in range(1, 1 + len(self.occupied_full_history)):
                        for task_id in range(len(self.occupied_full_history[-t])):
                            if (
                                self.occupied_full_history[-t][task_id]["var"][
                                    "routing"
                                ][(orbit_id + 1, sat_id + 1)][(s, d)]["trans_time"]
                                == t
                            ):
                                sat.pwq += self.occupied_full_history[-t][task_id][
                                    "task_data"
                                ][orbit_id][sat_id]
            ...
            link_ratio = self.B_evolving[1][(s, d)].value / self.graph[s][d]["full_bw"]
            avg_link_load_balancing += (link_ratio) ** 2
            lyap_MPC_MLLR = max(lyap_MPC_MLLR, link_ratio)
        avg_link_load_balancing /= len(self.graph.edges())
        ...
        for idx, dt in enumerate(self.future_alloc[0]):
            ...
            for sat in self.graph.nodes():
                ...
                if temp_bw > 1e-3:
                    ...
                    route_len = 0  # * calculate route len for propagation delay
                    for s, d in self.graph.edges():
                        route_len += (
                            self.cvx_MPC_comm_data[0][idx]["var"]["routing"][sat][
                                (s, d)
                            ].value
                            * self.graph[s][d]["distance"]
                        )
        ...
        avg_sat_load_balancing = 0
        lyap_MPC_MSP = 0
        for orbit_id in range(len(self.constellation.orbits)):
            orbit = self.constellation.orbits[orbit_id]
            for sat_id in range(len(orbit.sats)):
                sat = orbit.sats[sat_id]
                lyap_MPC_MSP = max(lyap_MPC_MSP, sat.pwq)
                avg_sat_load_balancing += ((sat.pwq / sat.fs) ** 2) / len(
                    self.graph.nodes()
                )
        ...
        return (
            avg_link_load_balancing,
            avg_sat_load_balancing,
            lyap_MPC_MLLR,
            lyap_MPC_MSP,
        )
            ...
            for task_id in range(len(self.cvx_MPC_comm_data[t])):
                ...
                for dst_sat in self.graph.nodes():  # * this loop for offloading dst
                    ...
                    if (task_data[dst_sat[0] - 1][dst_sat[1] - 1] > 1e-4) and (
                        ...
                        #! 4 constraints for auxiliary bilnear term
                        for s, d in self.graph.edges():
                            self.cvx_MPC_constraints += (
                                [
                                    self.cvx_MPC_comm_data[t][task_id]["var"][
    ...
    def cvx_create_MPC_obj(self):
        ...
        self.cvx_MPC_obj = cp.Minimize(
            (self.m1 * cp.sum_squares(loads) + self.m4 * cp.sum(route_len))
            / self.T_pred
        )
