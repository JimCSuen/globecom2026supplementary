...
class Satellite:
    ...
    def __init__(self, id, orbit):
        ...
        self.phase = (
            2 * np.pi * (id - 1) / orbit.num_sats - orbit.phase_offset
        )  # Phase of the satellite in the orbit in radians
        self.angular_velocity = np.sqrt(
            398600 / self.radius**3
        )  # * Angular velocity in rad/s, from kepler's law, correct
        self.rotation_matrix = (
            self._get_rotation_matrix()
        )  # Rotation matrix from the orbital plane to the ECEF coordinate system
        self.position_orbit = (
            self.calculate_position_orbit()
        )  # Position in the orbital plane (x, y, z) in km
        self.position_ecef = (
            self.calculate_position_ecef()
        )  # Position in the ECEF coordinate system (x, y, z) in km
        self.position_geodetic = (
            self.ecef_to_geodetic()
        )  # Position in geodetic coordinates (latitude, longitude, altitude)
    ...
    def get_info(self):
        ...
        return {
            ...
            "inclination (in degree)": np.degrees(
                self.inclination
            ),  # Inclination in degrees
            "right_ascension (in degree)": np.degrees(
                self.right_ascension
            ),  # Longitude of the ascending node in degrees
            "angular velocity (in rad/s)": self.angular_velocity,  # Angular velocity in rad/s
            "phase in orbit (in degree)": np.degrees(
                self.phase
            ),  # Phase in the orbit in degrees
    ...
    def calculate_position_ecef(self):
        ...
        - Numpy array of the position (x, y, z) in km.
        """
        position_ecef = np.dot(
            self.rotation_matrix, self.position_orbit
        )  # Rotate the position from the orbital plane to the ECEF coordinate system
...
class ServiceSatellite(Satellite):
    def __init__(self, id, orbit, fs):
        """
        Initialize the satellite's properties.
